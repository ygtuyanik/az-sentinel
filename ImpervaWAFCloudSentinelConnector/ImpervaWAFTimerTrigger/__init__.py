# Based on Azure-Sentinel ImpervaWAFCloudSentinelConnector
# https://github.com/Azure/Azure-Sentinel/tree/master/Solutions/ImpervaCloudWAF/Data%20Connectors/ImpervaWAFCloudSentinelConnector
# Modified: whitelist fields before send (slim_event) for ingestion cost reduction.
...
# Whitelist: parse_cef key -> Log Analytics column name (only these are sent to reduce cost)
TARGET_FIELDS = {
    "EventVendor": "EventVendor_s",
    "EventProduct": "EventProduct_s",
    "EventType": "EventType_s",
    "Device Version": "Device_Version_s",
    "Rulename": "Rulename_s",
    "Signature": "Signature_s",
    "Attack Name": "Attack_Name_s",
    "Attack Severity": "Attack_Severity_s",
    "sourceServiceName": "sourceServiceName_s",
    "siteid": "siteid_s",
    "Customer": "Customer_s",
    "start": "start_s",
    "end": "end_s",
    "request": "request_s",
    "requestMethod": "requestMethod_s",
    "cn1": "cn1_s",
    "app": "app_s",
    "act": "act_s",
    "qstr": "qstr_s",
    ...
    "src": "src_s",
    "requestClientApplication": "requestClientApplication_s",
    "latitude": "latitude_s",
...
def slim_event(ev: dict) -> dict:
    """Keep only whitelisted fields for Log Analytics (ingestion cost reduction)."""
    out = {}
    for src_key, dst_key in TARGET_FIELDS.items():
        val = ev.get(src_key)
        if val is not None and str(val).strip() != "":
            out[dst_key] = val
    # Time-generated-field header needs EventGeneratedTime
    evt_time = ev.get("EventGeneratedTime")
    if evt_time:
        out["EventGeneratedTime"] = evt_time
    return out
...
class ImpervaFilesHandler:
    ...
    def list_index_file(self):
        files_array = []
        try:
            r = self.session.get(url="{}/{}".format(self.url, "logs.index"),
                                 headers=self.auth
                                 )
            if 200 <= r.status_code <= 299:
                ...
                logging.error("Bad Request. The request was invalid or cannot be otherwise served. Error code: {}".format(r.status_code))
    ...
    def download_file(self, file_name):
        try:
            ...
            if 200 <= r.status_code <= 299:
                ...
                logging.error("Rate limit exceeded - Failed to download file {}. Response code: {}".format(file_name, r.status_code))
            else:
                if r.status_code is None:
                    logging.error("Something wrong. Error text: {}".format(r.text))
                else:
    ...
    def decrypt_and_unpack_file(self, file_name, file_content):
        ...
        if file_encryption_flag == -1:
            try:
                ...
                if 'while decompressing data: incorrect header check' in str(err):
            ...
            if events_data is not None:
                for line in events_data.splitlines():
                    if "CEF" in line:
                        ...
                        slim = slim_event(event_message)
                        if slim:
                            events_arr.append(slim)
        ...
        for elem in cs_array:
            try:
                if parsed_cef.get(elem) is not None:
                    ...
                    if parsed_cef.get(label_key):
                        parsed_cef[(parsed_cef[label_key]).replace(" ", "")] = parsed_cef[elem]
                    parsed_cef.pop(label_key, None)
                    parsed_cef.pop(elem, None)
            except Exception:
                pass
